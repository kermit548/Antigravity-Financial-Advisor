...
description: >
  ...
  圖像由 Antigravity 內建生圖工具（antigravity-draw）生成並 base64 內嵌，排版／文字／互動全部由 HTML + CSS + JS 處理，
...
以 SOIL 教學設計邏輯為骨架，用 **HTML + Antigravity 內建生圖圖像 + JS 互動** 產出單一 `.html` 簡報檔。
...
### 第 3 步：批次平行生圖（用 antigravity-draw 技能）

直接在對話中呼叫系統內建的 `generate_image` 工具進行生圖。
- **Prompt**（必要）：描述要生成的圖像內容。需寫明 `No readable text` 以避免亂碼文字，並留出文字疊放區域（例如 `with negative space at bottom for text overlay`）。
- **ImageName**（必要）：生成的圖像檔名（全小寫、底線連接，最多 3 個英文單字，例如 `slide_1_cover`）。
- **處理方式**：生圖結果會作為 Artifact 儲存在當前對話的 Artifacts 目錄中。請在生成後，利用檔案工具將產生的圖片複製或移動至 `slides/generated/` 專案目錄中。
...
- **antigravity-draw**（必要）：直接呼叫內建 `generate_image` 生圖工具平行批次生成所有頁面的 AI 圖
